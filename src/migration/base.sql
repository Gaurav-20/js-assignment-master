...
-- Had to manually create table as this sql file wasn't getting executed
create table if not exists mydb.news
(
    id int auto_increment not null primary key,
    title varchar(50) not null,
    description varchar(200) not null,
    matchId int, -- in case the news is for a tour, this may be NULL
    tourId int not null,
    sportId int not null,
...
-- Had to manually insert a few values as this sql file wasn't getting executed
-- Initial dummy values for get api testing
insert ignore into mydb.news (id, title, description, matchId, tourId, sportId) values (1, 'RCB close win against GT', 'RCB wins against GT by just 1 wicket to spare', 1, 1, 1);
insert ignore into mydb.news (id, title, description, matchId, tourId, sportId) values (2, 'Fantastic win for MI', 'MI won against CSK by whopping 98 runs', 2, 1, 1);
insert ignore into mydb.news (id, title, description, matchId, tourId, sportId) values (3, 'ISL Season comes to an end', 'ISL Season ended with Mumbai emerging as winners', NULL, 2, 2);